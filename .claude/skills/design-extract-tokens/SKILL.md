...
| --- | --- |
| **PlatformManager** (static prototype, chưa có framework) | **Nguồn chân lý:** khối `:root { ... }` DUY NHẤT trong thẻ `<style>` của `doc/Prototype/dashboard.html` — custom property: `--bg`, `--card`, `--text`, `--muted`, `--line`, `--brand`, `--brand2`, `--good`, `--warn`, `--bad`, `--shadow`. **KHÔNG có** Style Dictionary, không có file `*token*`, không có pipeline sinh token — đừng đi tìm. Cùng file còn chứa các class dùng token qua `var(--…)` (`.card`, `.btn.primary`, `.kpi .value`, `.badge.bdone/.bwork/.bstall`, …) — coi là global style thật. Typography: `Inter, Segoe UI, Arial, sans-serif` khai trong `body{font-family:...}`, không có scale rõ ràng — ghi lại đúng như đang có. |
| **Khi `src/FE/` có app thật** | Cập nhật lại dòng trên theo đúng vị trí token thật của framework được chọn (vd. CSS custom property trong file style toàn cục, theme file của UI library, v.v.) — không giả định trước framework nào. |
