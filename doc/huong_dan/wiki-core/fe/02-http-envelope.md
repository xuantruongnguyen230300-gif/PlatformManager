...
retryable, fields }` (xem `src/BE/.claude/rules/api-controller.md`
...
export const httpErrorInterceptor: HttpInterceptorFn = (req, next) =>
  next(req).pipe(
    catchError((err: HttpErrorResponse) => {
      ...
      return throwError(() => ({ ...err, apiResult: body }));
    }),
  );
```
...
Key của `fields` là **PascalCase** khớp property C# đã serialize (BE
`NormalizeField` giữ nguyên casing, xem `api-controller.md` §Exception-
handling) — component đọc `fields['MaxScore']`, không tự lowercase/camelCase
lại một cách ngầm định. Chi tiết bind vào form ở
[09-forms-validation.md](09-forms-validation.md).
