...
| Checkbox | `input[type="checkbox"]` | `16px` square, `accent-color: colors.brand`, `cursor:pointer` (`permission-matrix.scss:17-27`) | The `Phân quyền` permission matrix (`permission-matrix.html:22-28`). Also used unstyled (browser default) for `role-checkboxes` in the user dialog (`user-form-dialog.html:48`) and the sign-in "Ghi nhớ đăng nhập" box (`login.page.html:51`) |
...
| disabled | **Reachable only for checkboxes**, and only transiently: the permission matrix binds `[disabled]="loading()"` on every box, so the whole grid freezes while a fetch or save is in flight (`permission-matrix.html:25`, `phan-quyen.page.html:16`). No `:disabled` rule is authored for it — the browser default greying is what renders. **No text input or select in the app is ever `disabled` or `readonly`** — read-only screens hide the control instead (`danh-muc-dti.page.html:36-41` drops the whole action bar; `criteria-grid-table.html:66-79` renders plain text instead of an editable span) |
...
6. Checkbox styling is authored in exactly one place (`permission-matrix.scss:17-27`) and is component-scoped, so the user dialog's and the sign-in's boxes get none of it — two of the app's three checkbox call sites are unstyled browser default (`user-form-dialog.html:48`, `login.page.html:51`).
