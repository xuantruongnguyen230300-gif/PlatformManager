...
- **Divergences from the design-intent prototype** (`doc/Prototype/quan-tri-nguoi-dung.html`, intent only — not evidence of current behaviour): the prototype's three sortable column headers (`Người dùng ▾` / `Vai trò ▾` / `Ngày tạo ▾`, lines 246-249) ship **without sorting**, and its hand-rolled `‹ / ›` pager (lines 264-266) was replaced by the PrimeNG paginator with a rows-per-page dropdown. Decide deliberately whether column sorting is still wanted — if it is, it needs a server-side `sortBy`/`sortDir` contract, which `doc/contracts/users.md` does not currently define.
