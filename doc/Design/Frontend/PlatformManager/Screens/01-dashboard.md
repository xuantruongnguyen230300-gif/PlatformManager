...
> **Rewritten 2026-08-22.** Until this pass, this file described `doc/Prototype/dashboard.html` — a single-page `localStorage` app with inline editing and a floating save button. That file is **frozen history** (`doc/Design/CLAUDE.md` § Fidelity Policy, `doc/Prototype/README.md`) and is cited below only where it explains *why* a decision was made. Everything factual here was re-verified against `src/FE/src/app/modules/dashboard/`.
>
...
- **Page** — six stacked regions, **no page-level wrapper element**: the page component's template starts directly with the toolbar (`dashboard.page.html:1`)
  - `<app-period-toolbar>` → `Card` (Toolbar-card variant, `.weekbar.card.no-print`, `period-toolbar.html:1`) — horizontal flex, `gap sp-3`, `flex-wrap:wrap`, `margin-bottom sp-5` (`period-toolbar.scss:1-7`)
    ...
    - `SegmentedControl` (`.segmented` + `.seg-btn` × 2, `Components/SegmentedControl.md`, `period-toolbar.html:36-48`) — `inline-flex` with a shared 1px `line` border, `rounded.sm`, `overflow:hidden`, internal `border-left` divider, and an `.active` state filled `brand` on `on-primary` (`period-toolbar.scss:38-73`). It is **not** `TabBar` — that spec documents the separate `.btn`-based `.tabs-bar` on `/quan-tri/phan-quyen`
...
The shipped app loads **PrimeIcons v7** globally (`angular.json:38-39,124-125`), rendered as `<i class="pi pi-…">`. `Icons.md` was refreshed on 2026-08-22 and now records this correctly — the prototype-era `library: "none"` claim this spec used to repeat is gone.
...
- **The report dialog's backdrop does not close it.** Only the "Đóng" button and native `Esc` work (`report-dialog.html:1-11`) — no backdrop listener is wired, which is the same gap the prototype had (`doc/Prototype/dashboard.html`, recorded in this spec's previous revision) → wire backdrop-click-to-close, or state the "Đóng/Esc only" rule deliberately.
...
<!-- Undocumented-component pointers — RESOLVED 2026-08-22 by COMPONENTS.md pass C.
     ...
     the specs directly.
       ...
       - SegmentedControl-> WRITTEN, Components/SegmentedControl.md. Still NOT the same
                            control as TabBar.md — both ship, deliberately recorded.
