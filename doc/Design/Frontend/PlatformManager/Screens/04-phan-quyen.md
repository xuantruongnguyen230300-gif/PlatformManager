...
> **Not on this screen (yet): resource/action permissions.** `doc/contracts/permissions.md` also carries PERM-2 (`RolePermission`, `GET`/`PUT /api/admin/permissions/resources`), a *second* matrix intended to live beside this one. It is marked **`Status: DRAFT`** (`doc/contracts/permissions.md:59-64`). As of 2026-08-24 a component implementing it exists — `resource-permission-matrix` (`src/FE/src/app/platform/phan-quyen/components/resource-permission-matrix/`, uncommitted) — but it is **not wired into `phan-quyen.page.ts`** (which still imports only `PermissionMatrix`) and is referenced nowhere else in `src/FE/`. So this route still renders exactly as described below. It remains recorded under § Normalize on redesign — do **not** compose it into a screen, a prompt pack or a generated image until it is actually wired.
...
1. **Resource permissions are a plan, not a screen — and the design for them is not written.** `doc/contracts/permissions.md` PERM-2 (`RolePermission`, `GET`/`PUT /api/admin/permissions/resources`) describes a second role × row matrix, deny-by-default, with a `SuperAdmin` break-glass column, intended to sit beside the menu matrix on this route. It is **`Status: DRAFT`** (`doc/contracts/permissions.md:59-64`): the endpoint is not agreed. As of 2026-08-24 a component implementing it exists (`resource-permission-matrix.ts`/`.html`/`.scss`, uncommitted) but is **not wired into `phan-quyen.page.ts`** and referenced nowhere else — it does not render on this route yet. Consequences for design work: this route stays a single-matrix, tab-less card until PERM-2 ships wired in; a second matrix implies a view switcher, and the app's only switcher today is `SegmentedControl` on `/dashboard` (`Components/SegmentedControl.md`) — reuse it rather than inventing a `.btn`-based one. **Write the spec when the component is wired in and the endpoint moves off `DRAFT`, from the shipped code, not from the contract.** Until then no screen spec, prompt pack or generated image may show it. *(Recorded 2026-08-23 after a `TabBar` component spec and a whole second tab were found here describing UI that had never been built; updated 2026-08-24 — component code now exists but still unwired.)*
